...
public class Phong extends Lambertian {
	...
	for (Light light : scene.getLights()) {
	    ...
	    output.x += diffuseColor.x * light.color.x
		    * Math.max(0, vL.dot(normal));
	    output.y += diffuseColor.y * light.color.y
		    * Math.max(0, vL.dot(normal));
	    output.z += diffuseColor.z * light.color.z
		    * Math.max(0, vL.dot(normal));

	    output.x += specularColor.x * light.color.x
		    * Math.pow((Math.max(0, vE.dot(vR))), exponent);
	    output.y += specularColor.y * light.color.y
		    * Math.pow((Math.max(0, vE.dot(vR))), exponent);
	    output.z += specularColor.z * light.color.z
		    * Math.pow((Math.max(0, vE.dot(vR))), exponent);
