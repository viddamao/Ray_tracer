 ...
 * determined class type by setting the "type" attribute in the node.
 * Additionally, nodes can be named with the "name" attribute. After they are
 * added to the parent, named nodes are added to a hash table internal to the
 * parser. Later, other nodes can reference a named node by using a "ref"
 * attribute. The value of the reference attribute is used to index in the hash
 * table and the previously stored node with the matching name is used instead
 * of a parsed value.
 *
 * There are special routines for explicitly parsing primitives, arrays of
 * primitives, Tuple (either Vector or Point or Color), Strings, Images, and
 ...
 *
 * <scene> <camera> <eye> 1 1 1 </eye> </camera> <shader name="white"
 * type="Lambertian"> <diffuseColor>1 1 1</diffuseColor> </shader> <surface
 * type="Sphere"> <shader ref="white"/> <center>1 1 1</center>
 * <radius>1</radius> </surface> </scene>
 *
 ...
 * called in Camera -- Then setCamera(camera) is called on Scene -- The <shader>
 * tag is processed in the same fashion, but the type attribute specifies that
 * it be parsed additionally as a Lambertian shader. -- The name attribute in
 * the shader tag stores it for later use in the <surface> tag. The <shader> tag
 * in the <surface> tag is not parsed, but the same object produced by parsing
 * the above "white" shader is supplied instead.
 *
