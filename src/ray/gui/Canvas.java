...
public class Canvas extends JPanel {
	...
	private class Zoomer extends MouseAdapter implements MouseMotionListener {
		...
		public void mouseDragged(MouseEvent e) {
			...
			// constrain to be a square so we do not need to worry about
			// resizing the window
		...
		public void mouseReleased(MouseEvent e) {
			...
			if (zoomStart.x != zoomEnd.x && zoomStart.y != zoomEnd.y) {
				tracer.resizeViewport((double) zoomStart.x / getSize().width,
						(double) (getSize().height - zoomStart.y)
						/ getSize().height,
						(double) Math.abs(zoomEnd.x - zoomStart.x)
						/ getSize().width,
						(double) Math.abs(zoomEnd.y - zoomStart.y)
						/ getSize().height);
			}
		...
		public void mouseClicked(MouseEvent e) {
			...
			System.out.println(tracer.renderPixel(e.getX(), getSize().height
					- e.getY()));
		}
	}
}
